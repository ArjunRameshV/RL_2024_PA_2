...
for seed in seeds:
    ...
    acrobot_value_fn = REINFORCEValueNetwork(
        acrobot_hyperparameters["state_space"],
        seed,
        device
    )
    acrobot_optimizer = optim.Adam(acrobot_policy.parameters(), lr=acrobot_hyperparameters["lr"])
    acrobot_value_fn_optimizer = optim.Adam(acrobot_policy.parameters(), lr=acrobot_hyperparameters["vfn_lr"])

    agent = REINFORCEAgent(device, acrobot_policy, acrobot_optimizer, acrobot_value_fn, acrobot_value_fn_optimizer)
...
with open('acrobot_reinforce_type_2_reward_statistics.txt', 'w') as file:
...
plt.savefig('acrobot_reinforce_type_2.png')
plt.show()